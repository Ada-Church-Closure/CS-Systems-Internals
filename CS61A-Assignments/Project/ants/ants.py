    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.is_container and self.can_contain(place.ant):
                ...
                place.ant = self
                # 这里要注意只能保护一个的逻辑
            elif self.is_container == False and place.ant.is_container == True and place.ant.can_contain(self):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        index = 0
        while index < self.lower_bound and attack_place != None:
            index += 1
            attack_place = attack_place.entrance
        
        
        if attack_place == None:
            return None

        while attack_place.is_hive != True and index <= self.upper_bound:
            index += 1
            if len(attack_place.bees) == 0:
                if attack_place.entrance != None:
                    attack_place = attack_place.entrance
                else:
                    return None
            else:
                nearest_bee = random_bee(attack_place.bees)
                return nearest_bee
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        fucking_bees = self.place.bees
    
        for bee in fucking_bees[:]:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        ThrowerAnt.action(self, self.damage)
        double_place = self.place
        double_place = double_place.exit

        while double_place != None:
            if double_place.ant != None:
                ant1 = double_place.ant
                if ant1.is_doubled == False:
                    ant1.damage *= 2
                    ant1.is_doubled = True

                if ant1.is_container == True and ant1.ant_contained != None:
                    ant2 = ant1.ant_contained
                    if ant2.is_doubled == False:
                        ant2.damage *= 2
                        ant2.is_doubled = True
            double_place = double_place.exit
        # END Problem 12
